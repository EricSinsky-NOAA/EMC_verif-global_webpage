...
function getURL(domain,threshold,variable,frame,level,season,validtime){
	...
	newurl = newurl.replace("DDD",domain);
        newurl = newurl.replace("MMM",threshold);
...
function preload(obj){
	...
	for (i2=0; i2<hrs_loop.length; i2++){
		...
		var urls = getURL(obj.domain,obj.threshold,obj.variable,i,obj.level,obj.season,obj.validtime);
		...
		variables[idx_var].images[i].onerror = function(){remove_loading(this.varid,this.id); this.src='../../images/noimage.png';};
...
//	changeValid(curFrame);
        var curVar = searchByName(imageObj.season,seasons);
        if(curVar > 0){curVar = curVar - 1; changeSeason(seasons[curVar].name);}
...
//	changeValid(curFrame);
        var curVar = searchByName(imageObj.season,seasons);
        if(curVar < seasons.length-1){curVar += 1; changeSeason(seasons[curVar].name);}
}
...
function generate_url(){
	...
	append += "domain=" + imageObj.domain;
        ...
        //Add threshold
        append += "threshold=" + imageObj.threshold;  
