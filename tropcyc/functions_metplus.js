...
function pressDown(){
        var curVar = searchByName(imageObj.domain,domains);
        if(curVar < domains.length-1){curVar += 1; changeDomain(domains[curVar].name);}
}

function pressUp(){
        var curVar = searchByName(imageObj.domain,domains);
        if(curVar > 0){curVar = curVar - 1; changeDomain(domains[curVar].name);}
}
...
function updateMobile(){
	...
	|| navigator.userAgent.match(/Windows Phone/i)
		...
		document.getElementById('page-middle').innerHTML = "Swipe Left/Right on Image = Change Forecast Lead | Swipe Up/Down on Image = Change Storm";
	}
